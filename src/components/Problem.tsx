...
/* Четыре пункта были написаны в повелительном наклонении: «Убедите
   клиента», «Покажите», «Соберите», «Дайте». Читатель получал список
   своих будущих задач - то есть работу, которую ему предстоит сделать
   самому. А продаём мы обратное: что это сделаем мы.

   Теперь то же самое сказано от лица его клиента: не «убедите», а «клиент
   видит». Человек читает не поручение, а картину, в которой у него уже
   всё работает. */
const points = [
  { icon: Shield, num: '01', title: 'Доверие', desc: 'Клиент видит, что вы работаете не первый день, - ещё до звонка.' },
  { icon: Briefcase, num: '02', title: 'Услуги', desc: 'Человек понимает, что именно вы делаете, и не додумывает за вас.' },
  { icon: MessageSquare, num: '03', title: 'Заявки', desc: 'Контакт остаётся у вас, даже если человек пока не готов звонить.' },
  { icon: MapPin, num: '04', title: 'Контакты', desc: 'Связаться можно так, как удобно ему: звонок, мессенджер, карта.' },
];
...
export default function Problem() {
  return (
    <Container id="problem" pad="tight" className="relative border-t border-white/[0.06]">
      {/* Было: «Вы теряете клиентов, если бизнесу негде вызвать доверие».
          Мысль верная, но «бизнесу негде вызвать доверие» - канцелярская
          конструкция, через которую надо пробираться. Новый заголовок
          говорит то же самое механикой, которую человек узнаёт: выбор
          происходит до звонка, и если смотреть нечего - выбирают другого. */}
      <SectionTitle index="01" subtitle="Зачем бизнесу сайт" className="max-w-4xl">
        Вас выбирают до первого звонка. Если смотреть некуда - выбирают другого
      </SectionTitle>
      ...
      <div className="grid lg:grid-cols-2 gap-14 lg:gap-24 items-start mb-20 lg:mb-28">
        <Reveal className="space-y-6 text-[15px] md:text-base font-body text-fog leading-relaxed">
          {/* Первая фраза была «Вам нужен сайт для доверия, заявок и роста
              бизнеса». Утверждать потребность в лоб - самая слабая позиция:
              читатель ещё ни с чем не согласился, и его первая реакция -
              возразить. Теперь начинаем с его ситуации, которую он узнаёт,
              а вывод он делает сам. Это и есть разница между «нам нужно вам
              продать» и «мы понимаем, как у вас устроено». */}
          <p>
            Клиент может увидеть вас в рекламе, соцсетях, на картах или по рекомендации. Но перед обращением ему нужно быстро понять: кто вы, что предлагаете, почему вам можно доверять и как с вами связаться.
          </p>
          <p>
          ...
          <p className="text-bone font-medium border-l-2 border-cobalt pl-5 py-1 text-base md:text-lg">
            Пока этого нет, часть клиентов уходит к тем, у кого всё выглядит понятнее, надёжнее и удобнее. Молча - вы даже не узнаете, сколько их было.
          </p>
        </Reveal>
