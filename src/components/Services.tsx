...
export default function Services() {
  return (
    <Container id="pricing">
      ...
      <div className="mb-8 font-mono text-blue-500 uppercase tracking-widest text-sm border-b border-blue-900/50 pb-2 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]">
        Сайты по подписке (Месяц)
      </div>
      
      <div className="grid md:grid-cols-3 gap-8 mb-24 relative z-10">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-blue-900/10 via-transparent to-transparent pointer-events-none -z-10" />
        {products.map((p, i) => (
          <div key={i} className="flex flex-col bg-onyx-900/80 border border-onyx-700 hover:border-blue-500 transition-colors p-8 clip-diagonal group hover:shadow-[0_0_30px_rgba(30,58,138,0.3)]">
            <h3 className="text-2xl font-bold uppercase tracking-widest mb-2 group-hover:text-blue-400 transition-colors">{p.title}</h3>
            <p className="text-neutral-400 text-sm mb-6 pb-6 border-b border-onyx-700 group-hover:border-blue-900/50 transition-colors">{p.desc}</p>
            <div className="mb-8">
              <span className="text-5xl font-bold tracking-tighter text-white group-hover:text-blue-50 transition-colors">{p.price}</span>
              <span className="text-blue-500/70 font-mono text-sm ml-2">/мес</span>
            </div>
            ...
            <Button className="w-full group-hover:shadow-[0_0_20px_rgba(59,130,246,0.4)] transition-shadow">Оплатить</Button>
          </div>
        ))}
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
        {addons.map((a, i) => (
          <div key={i} className="bg-onyx-900 border border-onyx-800 p-6 flex flex-col hover:bg-onyx-800 hover:border-blue-900/50 transition-all group hover:shadow-[0_0_15px_rgba(30,58,138,0.2)]">
            <h4 className="font-bold uppercase tracking-widest mb-2 min-h-[48px] group-hover:text-blue-300 transition-colors">{a.title}</h4>
            <div className="text-xl font-bold tracking-tighter mb-4 text-white group-hover:text-blue-100 transition-colors">{a.price}</div>
            <p className="text-xs text-neutral-500 mb-6 flex-grow group-hover:text-neutral-400 transition-colors">{a.desc}</p>
            {a.action === 'Подробнее' ? (
              <Button variant="outline" className="w-full text-xs py-3 group-hover:bg-blue-900/20 group-hover:border-blue-500/50 group-hover:text-blue-400 transition-all">{a.action}</Button>
            ) : (
              <Button variant="ghost" className="w-full text-xs py-3 border border-onyx-700 group-hover:border-blue-900/50 group-hover:text-blue-400 transition-all">{a.action}</Button>
            )}
          </div>
        ))}
      </div>
