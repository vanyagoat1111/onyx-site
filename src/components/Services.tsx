...
const plans = [
  {
    name: 'Старт онлайн',
    scope: 'Одна страница · первая версия за 2-3 дня',
    subtitle: 'Быстрый выход в интернет: одна страница, которая объясняет, чем вы занимаетесь, и даёт способ с вами связаться',
    prices: { dev: '0 ₽', launch: '8 990 ₽', marketDev: '60 000 ₽' },
    features: [
  ...
  },
  {
    ...
    scope: 'Одна длинная страница · первая версия за 3-4 дня',
    subtitle: 'Сайт, задача которого - обращения. Структура выстроена так, чтобы человек дошёл до формы, а вы узнали об этом сразу',
    prices: { dev: '0 ₽', launch: '13 990 ₽', marketDev: '90 000 ₽' },
    features: [
  ...
  },
  {
    ...
    scope: 'Многостраничный сайт · первая версия за 5-7 дней',
    subtitle: 'Многостраничный сайт: отдельная страница под каждое направление, плюс заявки, аналитика и контроль обработки',
    prices: { dev: '0 ₽', launch: 'от 19 990 ₽', marketDev: '150 000 ₽' },
    features: [
      'всё из тарифа «Сайт + заявки»',
      'многостраничная структура: услуги, кейсы, о компании, контакты',
      'отдельная страница под каждое ключевое направление',
      'сквозная навигация и меню по разделам',
...
export default function Services() {
  return (
    <Container id="prices" pad="md" className="relative border-t border-white/[0.06] scroll-mt-20">
        ...
        {plans.map((plan, i) => {
          ...
          <Reveal key={i} delay={i * 0.1} className="h-full relative">
            ...
            <div className={`relative flex flex-col h-full rounded-[26px] border p-7 md:p-8 transition-all duration-500 overflow-hidden ${plan.badge ? 'border-cobalt/60 bg-gradient-to-b from-cobalt/[0.14] to-ink-2/80 shadow-[0_0_80px_rgba(78,124,255,0.18)]' : 'border-white/[0.08] bg-ink-2/60 hover:border-cobalt/30 hover:-translate-y-1 hover:shadow-[0_20px_60px_rgba(0,0,0,0.35)]'}`}>
              ...
              <h3 className="font-display font-semibold text-xl md:text-2xl text-white mb-1.5">{plan.name}</h3>
              {/* Объём и срок сразу под названием: без них человек сравнивает
                  одностраничник с многостраничным сайтом и не понимает разницу в цене. */}
              {plan.scope && (
                <p className="font-mono text-[11px] uppercase tracking-[0.12em] text-cobalt-soft mb-3">
                  {plan.scope}
                </p>
              )}
      ...
      {/* ── Оговорка к «0 ₽» (38-ФЗ ст.5) ── */}
      <Reveal className="relative z-10 -mt-8 mb-16">
        <p className="text-[12px] font-body text-fog/70 leading-relaxed max-w-3xl border-l-2 border-white/10 pl-4">
          «0 ₽» относится к стоимости разработки сайта. Отдельно оплачиваются запуск (домен, хостинг, публикация) и дополнительные опции - их актуальная стоимость указана выше и в разделе «Дополнительные опции». Итоговая сумма фиксируется в счёте до начала работ. Цены указаны в рублях и не являются публичной офертой в части индивидуальных доработок.
        </p>
      </Reveal>
