...
/* Шаг сборки первого экрана.
   ...
   Сцены сборки на странице больше нет - её сняли, потому что почти чёрный
   первый экран люди принимали за незагрузившийся сайт. Переменные и обёртки
   Step оставлены: у каждой стоит значение по умолчанию 1, то есть «готово»,
   поэтому без сцены всё видно сразу, а вернуть её можно одной строкой.

   Как это работало: вступление писало прогресс в CSS-переменные на :root,
...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div
        data-intro-fit
        /* Верхний отступ обязан перекрывать шапку.

           Шапка у нас position: fixed - она вынута из потока и ничего
           под собой не двигает. При py-5 её высота около 84 точек,
           а отступ начинался от 56: на невысоком окне заголовок и метка
           уезжали под меню.

           Раньше это прикрывала сцена сборки: она сдвигала содержимое
           вниз на высоту своей панели. Сцену сняли - и подпорка ушла
           вместе с ней. Здесь как раз тот случай, когда правка в одном
           месте открыла дыру в другом.

           Нижняя граница clamp теперь 96 точек: перекрывает 84 с запасом
           на любом экране. Верхняя оставлена свободной, чтобы на высоком
           мониторе первый экран дышал. */
        className="relative pt-[clamp(6rem,12vh,8rem)] pb-[clamp(1.5rem,4vh,4rem)]"
        style={{
      ...
      <div className="relative z-10 max-w-[1440px] w-full mx-auto px-5 sm:px-6 md:px-12">
        ...
        <div className="max-w-3xl">
          ...
          >
            {/* Порядок кнопок изменён: первым идёт «сайта нет».

                Раньше главной кнопкой был аудит сайта, а он по своей природе
                требует адрес. То есть самый заметный элемент первого экрана
                отсекал того, ради кого построена вся воронка: действующий
                бизнес БЕЗ сайта - это основной клиент студии.

                Человеку без сайта приходилось сначала опознать себя в
                неглавной кнопке с формулировкой «пока нет», то есть через
                отрицание и вторым номером. Теперь его вход первый и назван
                через действие, а не через нехватку. */}
            <div className="flex flex-col gap-2">
              <Button className="min-h-[56px] px-9" onClick={() => setFormOpen(true)}>
                Сайта нет - с чего начать
              </Button>
              <span className="text-[11px] text-fog/70 text-center font-mono tracking-wide max-w-[280px] mx-auto">10 минут: путь вашего покупателя, структура сайта и цена.</span>
            </div>
            <div className="flex flex-col gap-2">
              {/* deep-link открывает разбор сайта сразу, без приветствия. */}
              <Button variant="outline" className="min-h-[56px] px-9" onClick={() => window.open(BOT_AUDIT_LINK, '_blank')}>
                Сайт есть - разберите его
              </Button>
              {/* Цифра здесь и цифра в инфографике справа - это одна и та же
                  цифра. Они стоят в двадцати пикселях друг от друга, и «за
