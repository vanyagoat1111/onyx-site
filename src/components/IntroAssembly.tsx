...
/* ── Звёзды пустого экрана ──

   Пустая страница не должна быть просто чёрным прямоугольником: чистое
   ничто читается как «не загрузилось». Россыпь искр даёт понять, что это
   решение, а не ошибка, и заодно делает первый кадр узнаваемым.

   Форма - четырёхлучевая искра с вогнутыми лучами, а не пятиконечная
   звёздочка: это огранка, блик на камне, отсюда и название студии.

   Координаты заданы списком, а не случайными числами. Случайность здесь
   означала бы, что при каждой отрисовке узор другой, и половина искр
   рано или поздно окажется прямо под заголовком.

   Гаснут раньше, чем приходит заголовок (0.08-0.38 против 0.25): это
   украшение пустоты, и на собранной странице ему делать нечего. Поэтому
   и значение по умолчанию у переменной - ноль, а не единица: если скрипт
   не выполнился, человек видит готовый сайт без искр, а не готовый сайт
   с непонятными блёстками поверх текста. */
const STARS: [number, number, number, number, boolean][] = [
  // x% ширины, y% высоты окна, размер px, задержка мерцания s, светлая ли
  [11, 20, 20, 0.0, false], [26, 63, 11, 1.1, false], [41, 13, 13, 0.5, true],
  [55, 79, 9, 2.0, false], [68, 29, 24, 0.8, false], [79, 57, 12, 1.6, true],
  [89, 17, 10, 2.4, false], [6, 73, 14, 1.9, false], [34, 42, 8, 0.3, false],
  [61, 51, 10, 2.7, true], [93, 67, 16, 1.3, false], [18, 44, 9, 2.2, false],
  [47, 88, 11, 0.9, false], [73, 85, 8, 1.5, false], [86, 40, 7, 3.1, false],
];
  ...
  return (
    ...
    <div ref={track} data-intro-track className="relative" style={{ height: trackH }}>
      <div ref={pane} className="sticky top-0" style={{ height: paneH }}>
        ...
        {/* ── искры пустого экрана ──
            Мерцание на CSS, без единой строчки на кадр: анимируются только
            opacity и transform, то есть для видеокарты это бесплатно.
            Слой целиком гаснет одной переменной, поэтому пятнадцать искр
            стоят столько же, сколько одна. */}
        <div
          /* Без z-index: слой лежит первым в разметке и потому под содержимым.
             Искры гаснут к 0.38, а заголовок приходит на 0.25 - если поднять
             их наверх, в это окно блёстки окажутся поверх текста. Фон пустой
             страницы должен оставаться фоном. */
          className="absolute inset-0 pointer-events-none"
          style={{ opacity: 'var(--intro-stars, 0)' }}
          aria-hidden="true"
        >
          {STARS.map(([x, y, s, d, light], i) => (
            <span
              key={i}
              className="onyx-star absolute"
              style={{
                left: `${x}%`,
                top: Math.round((y / 100) * vh),
                width: s,
                height: s,
                marginLeft: -s / 2,
                marginTop: -s / 2,
                color: light ? 'rgba(242,240,233,0.5)' : 'rgba(78,124,255,0.55)',
                animationDelay: `${d}s`,
              }}
            >
              {/* Четырёхлучевая искра с вогнутыми лучами: огранка камня,
                  а не детская звёздочка из пяти концов. */}
              <svg viewBox="-10 -10 20 20" className="w-full h-full block">
                <path
                  d="M0-10C.9-3.6 3.6-.9 10 0 3.6.9.9 3.6 0 10-.9 3.6-3.6.9-10 0-3.6-.9-.9-3.6 0-10Z"
                  fill="currentColor"
                />
              </svg>
            </span>
