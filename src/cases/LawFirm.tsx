...
/* Цифры показателей заданы в двух местах - на первом экране и в полосе
   ниже. Раньше они расходились: 94% «выигранных дел» вверху и 92%
   «успешных дел» внизу. Человек, который читает страницу целиком,
   замечает такое сразу, и дальше не верит уже ни одной цифре.

   Расхождение внутри одного сайта бьёт по доверию сильнее, чем полное
   отсутствие цифр. Здесь и там теперь одно значение и одна формулировка. */
const goldStats = [
  { value: '15+', label: 'Лет практики' },
  { value: '94%', label: 'Выигранных дел' },
  ...
  return (
    <div className="relative min-h-screen bg-[#0A1023] text-slate-300 font-cormorant selection:bg-[#C9A263]/25 selection:text-white overflow-x-clip tex-paper">
      ...
      <section className="relative min-h-[86dvh] flex items-center py-20 md:py-24 px-6 md:px-8 overflow-hidden">
        {/* ── Первый экран переписан на отказ, а не на превосходство ──

            Было: «Безупречная защита интересов вашего бизнеса в суде».
            Фраза, под которой подпишется любое бюро страны, - и ровно
            поэтому она ничего не сообщает. «Безупречная» вдобавок
            обещает результат, который в суде гарантировать нельзя.

            Человек с корпоративным спором боится не того, что защита
            будет «не безупречной». Он боится взять юриста, который
            возьмёт деньги за заведомо проигрышное дело. Это главный
            страх ниши, и он не про качество - он про честность.

            Поэтому заголовок продаёт готовность сказать «нет». Обещание,
            которое конкуренты повторить не смогут, не потеряв часть
            выручки, - и потому оно работает.

            Точка вынесена в акцентный цвет вместо слова: цветное слово
            внутри заголовка перетягивает смысл на себя, а здесь важна
            вся фраза целиком. Точка держит паузу перед второй строкой,
            и та читается как то, что человек не ожидал услышать. */}
        ...
        <div className="max-w-[1360px] mx-auto relative z-[2] w-full grid grid-cols-1 lg:grid-cols-[1.05fr_0.95fr] gap-12 lg:gap-16 xl:gap-20 items-center">
          ...
          <div>
            ...
            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.1, ease: EASE }} className="text-[34px] sm:text-[46px] md:text-[58px] xl:text-[64px] leading-[1.1] text-white font-medium mb-7 [text-wrap:balance]">
              Скажем сразу, есть ли у дела перспектива<span className="text-[#C9A263]">.</span> Даже если ответ вам не понравится
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.2, ease: EASE }} className="font-manrope text-base md:text-[17px] font-light leading-[1.75] max-w-[520px] mb-10">
              Корпоративные споры, защита активов и сопровождение сделок. На первой встрече разбираем документы и называем шансы честно - браться за безнадёжное дело ради гонорара мы не станем.
            </motion.p>
