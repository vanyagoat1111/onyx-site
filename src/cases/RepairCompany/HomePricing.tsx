  ...
  return (
    <Section id="packages" className="bg-darker border-t border-gold/10">
      <div className="mb-16 md:mb-24 border-b border-gold/20 pb-8 text-center">
        ...
        <p className="mt-6 font-sans font-light text-ivory/60 max-w-2xl mx-auto text-[13px] tracking-wide leading-relaxed">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        {packages.map((pkg, i) => (
          <div key={i} className={`flex flex-col border p-10 transition-colors duration-500 ${pkg.highlight ? 'border-gold/60 bg-gold/5 hover:border-gold' : 'border-gold/20 hover:border-gold/50'}`}>
            ...
            <p className="font-sans font-light text-[13px] tracking-wide text-ivory/60 mb-10 h-16 leading-relaxed">{pkg.desc}</p>
            
            <ul className="space-y-4 mb-12 flex-1">
              {pkg.features.map((f, idx) => (
                <li key={idx} className="flex items-start font-sans font-light text-[13px] text-ivory/80">
            ...
            <div key={i} className="border-b border-gold/20">
              ...
              <AnimatePresence>
                {openIdx === i && (
                  ...
                  >
                    <p className="font-sans font-light text-[13px] text-ivory/60 pb-8 leading-relaxed tracking-wide">
                      {faq.a}
                    </p>
                  </motion.div>
                )}
    ...
    <Section id="contact" className="bg-darker">
      <div className="grid md:grid-cols-2 gap-16 lg:gap-24">
        <div>
          ...
          <p className="font-sans font-light text-ivory/60 mb-12 text-[13px] tracking-wide leading-relaxed">
            Оставьте заявку, и наш ведущий специалист свяжется с вами для обсуждения деталей вашего будущего интерьера.
          </p>

          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="relative">
              <input required type="text" placeholder="Ваше имя" className="w-full bg-transparent border-b border-gold/30 py-4 outline-none focus:border-gold transition-colors font-sans text-ivory placeholder:text-ivory/30" />
            </div>
            <div className="relative">
              <input required type="tel" placeholder="Номер телефона" className="w-full bg-transparent border-b border-gold/30 py-4 outline-none focus:border-gold transition-colors font-sans text-ivory placeholder:text-ivory/30" />
            </div>
            <div className="relative">
              <input type="text" placeholder="Тип помещения (квартира, дом, офис)" className="w-full bg-transparent border-b border-gold/30 py-4 outline-none focus:border-gold transition-colors font-sans text-ivory placeholder:text-ivory/30" />
            </div>
        ...
        <div className="relative h-[500px] md:h-auto min-h-[500px]">
          <div className="absolute inset-0 bg-dark border border-gold/20 flex flex-col items-center justify-center p-8 text-center overflow-hidden">
            <div className="absolute inset-0 flex items-center justify-center opacity-20">
              <div className="w-[400px] h-[400px] rounded-full border border-gold/50 absolute animate-ping" style={{ animationDuration: '3s' }} />
              <div className="w-[300px] h-[300px] rounded-full border border-gold/50 absolute" />
              <div className="w-[200px] h-[200px] rounded-full border border-gold/50 absolute" />
              <div className="w-[100px] h-[100px] rounded-full border border-gold/50 absolute bg-gold/10" />
              
              <div className="absolute w-[500px] h-[1px] bg-gold/30 rotate-45" />
              <div className="absolute w-[500px] h-[1px] bg-gold/30 -rotate-45" />
              <div className="absolute w-[500px] h-[1px] bg-gold/30" />
              <div className="absolute w-[1px] h-[500px] bg-gold/30" />
            </div>

            <div className="relative z-10 p-12 backdrop-blur-md bg-dark/70 border border-gold/20 hover:border-gold/50 transition-colors duration-500">
              <h3 className="font-serif text-2xl mb-4 italic">Наш шоурум</h3>
              <p className="font-sans font-light text-[13px] tracking-wide text-ivory/80 mb-2">г. Москва, Пресненская наб., 12</p>
              <p className="font-sans font-light text-[13px] tracking-wide text-ivory/60 mb-8">Башня Федерация, 45 этаж</p>
