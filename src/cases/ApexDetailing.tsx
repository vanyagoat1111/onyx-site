  ...
  const car = (
    ...
    >
      ...
      >
        ...
        {/* Кадр на телефоне сдвинут к переду машины.

            Снимок горизонтальный - 1200 на 626. Коробка под него занимает
            верхнюю половину экрана и по пропорциям почти квадратная, поэтому
            в неё влезает лишь около 46 процентов ширины кадра. Остальное
            обрезается, и вопрос только в том, какая часть машины останется.

            По центру оставался бок: дверь, порог, заднее крыло. Узнать
            автомобиль по такому куску нельзя.

            objectPosition сдвинут к восьми процентам - это левый край кадра,
            где стоит перед: решётка, фары, эмблема, переднее колесо. Считано
            по снимку: кузов занимает от 5 до 78 процентов ширины, окно при
            восьми процентах открывает участок с 4 по 51 - весь нос помещается
            с небольшим запасом, и обрез приходится на середину двери.

            На компьютере коробка широкая, кадр входит почти целиком,
            и сдвиг там не нужен. */}
        <img
          src={mobile ? CAR_PHOTO_SM : src} alt="" aria-hidden="true"
          className="absolute inset-0 w-full h-full object-cover"
          style={{
            objectPosition: mobile ? '8% 50%' : 'center',
